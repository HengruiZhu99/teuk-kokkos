...
void evaluate_plus2_spatial_ordered_pairs(
  ...
  Kokkos::parallel_for(
      "teuk_plus2_spatial_ordered_pairs",
      ...
      KOKKOS_LAMBDA(const std::size_t flat) {
        ...
        const auto primitive = [&](const std::size_t mode,
                                   const Plus2SpatialPrimitive component) {
          const std::size_t c = static_cast<std::size_t>(component);
          return Jet1<Complex>{primitive_value(mode, c, radial, theta),
                               primitive_tangent(mode, c, radial, theta)};
        };
        const auto derivative = [&] (
            const std::size_t mode,
            const Plus2SpatialPairDerivative component) {
          const std::size_t c = static_cast<std::size_t>(component);
          return Jet1<Complex>{derivative_value(mode, c, radial, theta),
                               derivative_tangent(mode, c, radial, theta)};
        };
        ...
        for (std::size_t pair = pair_offsets(target_slot);
          ...
          const Plus2OrderedPairFieldsT<Jet1<Complex>> fields{
              primitive(m1, Plus2SpatialPrimitive::V),
              primitive(m1, Plus2SpatialPrimitive::C),
              jet_conj(primitive(sharp1, Plus2SpatialPrimitive::C)),
              primitive(m1, Plus2SpatialPrimitive::B),
              jet_conj(primitive(sharp1, Plus2SpatialPrimitive::B)),
              primitive(m1, Plus2SpatialPrimitive::Sig),
              primitive(m1, Plus2SpatialPrimitive::Kap),
              primitive(m1, Plus2SpatialPrimitive::Rh),
              jet_conj(primitive(sharp1, Plus2SpatialPrimitive::Rh)),
              primitive(m1, Plus2SpatialPrimitive::Ep),
              jet_conj(primitive(sharp1, Plus2SpatialPrimitive::Ep)),
              primitive(m2, Plus2SpatialPrimitive::Z0),
              primitive(m2, Plus2SpatialPrimitive::Z1),
              primitive(m2, Plus2SpatialPrimitive::H),
              primitive(m2, Plus2SpatialPrimitive::Sig),
              primitive(m2, Plus2SpatialPrimitive::Kap)};
          const Plus2OrderedPairDerivativesT<Jet1<Complex>> derivatives{
              derivative(m2, Plus2SpatialPairDerivative::CapitalDelta4Z1),
              derivative(m2, Plus2SpatialPairDerivative::EthPrime4Z1),
              derivative(m1,
                         Plus2SpatialPairDerivative::CapitalDelta2CSharp),
              derivative(m1, Plus2SpatialPairDerivative::EthPrime1BSharp),
              derivative(m2, Plus2SpatialPairDerivative::CapitalDelta5Z0),
              derivative(m2, Plus2SpatialPairDerivative::Eth5Z0),
              derivative(m1, Plus2SpatialPairDerivative::CapitalDelta2C),
              derivative(m1, Plus2SpatialPairDerivative::Eth1B),
              derivative(m1, Plus2SpatialPairDerivative::CapitalDelta2V),
              derivative(m1, Plus2SpatialPairDerivative::Eth2C),
              derivative(m1,
                         Plus2SpatialPairDerivative::EthPrime2CSharp),
              derivative(m2, Plus2SpatialPairDerivative::CapitalDelta2Sig),
              derivative(m2, Plus2SpatialPairDerivative::CapitalDelta3Kap),
              derivative(m2, Plus2SpatialPairDerivative::EthPrime3Kap)};
          ...
          for (std::size_t family = 0;
               ...
               static_cast<std::size_t>(Plus2SpatialPairFamily::Count);
               ++family) {
            pair_value(pair, family, radial, theta) = families[family].value;
            pair_tangent(pair, family, radial, theta) = families[family].dt;
          }
          summed_J += families[static_cast<std::size_t>(
              Plus2SpatialPairFamily::J)];
          summed_K += families[static_cast<std::size_t>(
              Plus2SpatialPairFamily::K)];
          summed_Q += families[static_cast<std::size_t>(
              Plus2SpatialPairFamily::Q)];
        }
        ...
        for (std::size_t aggregate = 0;
             ...
             static_cast<std::size_t>(Plus2SpatialAggregate::Count);
             ++aggregate) {
          summed_value(target, aggregate, radial, theta) =
              sums[aggregate].value;
          summed_tangent(target, aggregate, radial, theta) =
              sums[aggregate].dt;
        }
      });
}
...
void evaluate_plus2_spatial_outer_source(
  ...
  const std::size_t target_points =
      workspace.target_count() * radial_points * theta_points;
  Kokkos::parallel_for(
      "teuk_plus2_spatial_outer_source",
      Kokkos::RangePolicy<ExecutionSpace>(execution, 0, target_points),
      KOKKOS_LAMBDA(const std::size_t flat) {
        const std::size_t plane = radial_points * theta_points;
        const std::size_t target_slot = flat / plane;
        ...
        const auto aggregate = [&](const Plus2SpatialAggregate component) {
          const std::size_t c = static_cast<std::size_t>(component);
          return Jet1<Complex>{projected_sum_value(mode, c, radial, theta),
                               projected_sum_tangent(mode, c, radial, theta)};
        };
        const auto derivative = [&] (
            const Plus2SpatialOuterDerivative component) {
          const std::size_t c = static_cast<std::size_t>(component);
          return Jet1<Complex>{outer_derivative_value(mode, c, radial, theta),
                               outer_derivative_tangent(mode, c, radial,
                                                        theta)};
        };
        const KerrBackgroundPoint background = kerr_background_point(
            parameters, radius, cos_theta(theta), sin_theta(theta));
        ...
        const Jet1<Complex> activated =
            source_activation_multiplier * raw.total();
        ...
        source_value(mode, radial, theta) = activated.value;
        source_tangent(mode, radial, theta) = activated.dt;
        forcing_value(mode, radial, theta) = forcing.value;
        forcing_tangent(mode, radial, theta) = forcing.dt;
      });
}
