...
// Fifth-order promoted q0 extraction used only by the complete Route-B
// curvature graph.  The minimum six-node authority above is retained and
// independently tested; this seven-node form additionally annihilates R^6 so
// the predeclared N=9/17/33 componentwise gate reaches the asymptotic regime.
template <class Value>
KOKKOS_INLINE_FUNCTION Value plus2_extract_q0_promoted_at_scri(
    const Value* f0, const std::size_t point_count,
    const double inverse_spacing, const std::size_t stride = 1) {
  if (point_count < plus2_q0_promoted_endpoint_nodes + 1 ||
      !(inverse_spacing > 0.0)) {
    return Value{};
  }
  Value result{};
  for (std::size_t node = 0; node < plus2_q0_promoted_endpoint_nodes;
       ++node) {
    result += plus2_q0_promoted_endpoint_weights[node] *
              f0[(node + 1) * stride];
  }
  return inverse_spacing * inverse_spacing * result;
}
